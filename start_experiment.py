    ...
    if (mode == "train"):
        ...
        for node_type in ['FileObject']:
            ...
            if len(node_features) > 0:
                ...
                feature_array = []
                for i, item in enumerate(ori_feature_array):
                    feature_array.append(np.zeros(10002))
                    feature_array[-1][oh_index[i]] = 1
                    feature_array[-1][10000] = item[1]
                    feature_array[-1][10001] = item[2]
            else:
                model_nids[node_type] = []
                feature_array = []
            model_features[node_type] = torch.tensor(feature_array, dtype=torch.int64)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_data", nargs='?', default="/home/weijian/weijian/projects/E3data/ta1-trace-e3-official-1.json", type=str)
    parser.add_argument("--volume_num", nargs='?', default=7, type=int)
    parser.add_argument("--test_data", nargs='?', default="/home/weijian/weijian/projects/E3data/ta1-trace-e3-official-1.json", type=str)
