...
with open('adversarial/artifacts/mimicry_logs.json', 'w') as outfile:
    ## Open two files and the output file
    with open('../data/raw/ta1-cadets-e3-official-2.json/ta1-cadets-e3-official-2.json', 'r') as file1:
        for line1 in tqdm(file1):
            if line1.endswith('\n') == False:
                line1 += '\n'
            outfile.write(line1)

    with open('../data/raw/ta1-cadets-e3-official.json/ta1-cadets-e3-official.json', 'r') as file2:
        for line2 in tqdm(file2):
            ...
            # else:
            #     outfile.write(line2)
            if "com.bbn.tc.schema.avro.cdm18.Event" not in line2:
                if line2.endswith('\n') == False:
                    line2 += '\n'
                outfile.write(line2)
    
    with open('../data/raw/ta1-cadets-e3-official.json/ta1-cadets-e3-official.json.1', 'r') as file2:
        for line2 in tqdm(file2):
            data2 = json.loads(line2)
            if "com.bbn.tc.schema.avro.cdm18.Event" in data2["datum"]:
