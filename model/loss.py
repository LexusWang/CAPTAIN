...
def get_loss(event_type: str, s: torch.Tensor, o: torch.Tensor, alarm_name: str, side, format = 'cdm'):
    ...
    if side == "false_positive":
        if event_type == standard_events['EVENT_EXECUTE'] or event_type == standard_events['EVENT_LOADLIBRARY']:
            s_loss = torch.tensor([s[0]-TRUSTED, 0, 0, 0, 0])

        elif event_type == standard_events['EVENT_MODIFY_PROCESS']:
            o_loss = torch.tensor([o[0]-TRUSTED, 0, 0, 0, 0])
            ...
            o_loss = torch.tensor([0, 0, 0, o[3]-1, 0])
            if alarm_name == "DataLeak":
                s_loss = torch.tensor([0, 0, 0, s[3]-1, s[4]-1])

        elif event_type == standard_events['EVENT_CHANGE_PRINCIPAL']:
            s_loss = torch.tensor([0, 0, 0, s[3]-1, 0])
            ...
            s_loss = torch.tensor([0, 0, 0, o[3]-1, 0])

    elif side == "false_negative":
        if event_type == standard_events['EVENT_EXECUTE'] or event_type == standard_events['EVENT_LOADLIBRARY']:
            s_loss = torch.tensor([s[0]-UNTRUSTED, 0, 0, 0, 0])

        elif event_type == standard_events['EVENT_MODIFY_PROCESS']:
            o_loss = torch.tensor([o[0]-UNTRUSTED, 0, 0, 0, 0])

        elif event_type in {standard_events['EVENT_WRITE'],standard_events['EVENT_SENDMSG']}:
            o_loss = torch.tensor([0, 0, 0, o[3], 0])
            if alarm_name == "DataLeak":
                s_loss = torch.tensor([0, 0, 0, s[3], s[4]])

        elif event_type == standard_events['EVENT_CHANGE_PRINCIPAL']:
            s_loss = torch.tensor([0, 0, 0, s[3], 0])

        elif event_type == standard_events['EVENT_MODIFY_FILE_ATTRIBUTES']:
            o_loss = torch.tensor([0, 0, 0, o[3], 0])

        elif event_type in {standard_events['EVENT_MPROTECT'], standard_events['EVENT_MMAP']}:
            s_loss = torch.tensor([0, 0, 0, o[3], 0])
